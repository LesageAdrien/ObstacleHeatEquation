...
class Simulator:
    def __init__(self, Nx, Ny, circlepos = (0.5, 0.5), circleradius = 0.1):
        ...
        diag = np.ravel(np.sqrt((dx(self.PHI) * self.Nx) ** 2 + (dx(self.PHI.T).T * self.Ny) ** 2))

        self.MAT2 = sps.diags([diag], [0], dtype=float, format="csr")
        self.LMBDSIZE = np.sum((diag > 1e-14).astype(int))
        self.MAT2 = self.MAT2[:, diag > 1e-14 ]
        self.S = np.ravel(self.S)

        self.MAT_DUAL = sps.bmat([[self.MAT1, self.MAT2], [self.MAT2.T, None]])
        self.EYEDUAL = sps.bmat([[self.eye, self.MAT2*0], [self.MAT2.T*0, None]])


    def tick(self, dt):
        self.U = np.reshape(sps.linalg.spsolve(self.EYEDUAL - dt * self.MAT_DUAL, np.hstack((np.ravel(self.U) + dt*self.S, np.zeros(self.LMBDSIZE))))[:self.Nx*self.Ny], (self.Nx, self.Ny))
        #self.U += dt * np.reshape(self.MAT1.dot(np.ravel(self.U)) + self.S, (self.Nx, self.Ny))
